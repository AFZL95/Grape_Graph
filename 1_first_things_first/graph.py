...
# "AdjacencySetGraph" inherits from the "Graph" abstract class
class AdjacencySetGraph(Graph):
    def __init__(self, num_vertices, directed=False):
        ...
        self.vertex_list = []
        # initialize every node with it's unique id
        for i in range(num_vertices):
            self.vertex_list.append(Node(i))

        # implentation of other abstract methods in the super method "Graph"
    ...
    def get_edge_weight(self, v1, v2):
        # it could be changed when were a need for weighted graph !
        return 1
...
class AdjacencyMatrixGraph(Graph):
    ...
    def get_in_degree(self, v):
        # check vortex validation as usuall
        if v < 0 or v >= self.num_vertices:
            raise ValueError("Cannot access vertex %d" % v)
...
for i in range(4):
    print("In_degree: ", i, g.get_in_degree(i))
